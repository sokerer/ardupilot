...
/*
  wrapper class for I2C to expose protected functions from VRNuttX
 */
...
{
    return vrbrain_i2c->do_transfer(addr, data, len, nullptr, 0) ? 0:1;
}
...
{
    uint8_t d[2] = { reg, val };
    return vrbrain_i2c->do_transfer(addr, d, sizeof(d), nullptr, 0) ? 0:1;
}
...
{
    return vrbrain_i2c->do_transfer(addr, nullptr, 0, data, len) ? 0:1;
}
...
{
    return vrbrain_i2c->do_transfer(addr, &reg, 1, data, 1) ? 0:1;
}
...
uint8_t VRBRAINI2CDriver::readRegisters(uint8_t addr, uint8_t reg,
                                    uint8_t len, uint8_t* data)
{
    return vrbrain_i2c->do_transfer(addr, &reg, 1, data, len) ? 0:1;
}
